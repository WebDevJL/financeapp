...
/******************************************************************************************************************************************/
		...
		//Check local storage
		var tmpStr_1 = $.jStorage.get("AccountsBalance_Str_Offline", '');
		var tmpStr_2 = $.jStorage.get("Budgets_Str_Offline", '');
		...
		if(htmlStr_AccountBalance === ''){
			...
			var i=0, j=0;
			//Get filters data
			var url = location.protocol + "//" + location.host + "/FinanceApp/Files/accounts.xml";
			...
			url = location.protocol + "//" + location.host + "/FinanceApp/Files/Offline/transactions_"+cid+".xml";
			xml = new JKL.ParseXML( url );
			var obj2 = xml.parse();
			console.log(obj2);
			for (var i = 0; i < obj2.transactions.transaction.length; i++){//loop through each transaction
				if((obj2.transactions.transaction[i].enable)*1 > 0) {
		...
		if(tmpStr_1 === ''){$.jStorage.set("AccountsBalance_Str_Offline", htmlStr_AccountBalance, {TTL : 600});}
/******************************************************************************************************************************************/
/* Fill budget table*/
/******************************************************************************************************************************************/
		if(htmlStr_Budgets === ''){
			...
			url = location.protocol + "//" + location.host + "/FinanceApp/Files/budgetSettings.xml";
			xml = new JKL.ParseXML( url );
			var obj3 = xml.parse();
			...
			for (var i = 0; i < obj2.transactions.transaction.length; i++){//loop through each transaction
				...
				if(obj2.transactions.transaction[i].currency === cid  && (obj2.transactions.transaction[i].enable)*1 > 0){
					for (var j = 0; j < obj3.budgets.budget.length; j++){//loop through each budget
						if (obj3.budgets.budget[j].items.item.length === undefined){
			...
			for (var j = 0; j < accounts.length; j++){
				var accountCheck = 0;
				if(accounts[j].id === "2" || accounts[j].id === "3" || accounts[j].id === "4" || accounts[j].id === "5" || accounts[j].id === "7" || accounts[j].id === "8"){accountCheck=1;}
				for (var k = 0; k < obj3.budgets.budget.length; k++){
					...
					if (obj3.budgets.budget[k].items.item.length === undefined && j === 0 && obj3.budgets.budget[k].id !== "26"){
		...
		//Store output string in local storage
		if(tmpStr_2 === ''){$.jStorage.set("Budgets_Str_Offline", htmlStr_Budgets, {TTL : 600});}
	});
});