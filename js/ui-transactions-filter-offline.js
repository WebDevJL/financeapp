...
//hide dialogs DIVs
	...
	$('#filterXML_Offline').click(function() {
		$('#transactionsXML_Offline').empty();
		//Get filters data
		var sDate = new Date("1900-01-01"), eDate = new Date("2100-01-01"), acc = "", tt = "", cat = "", scat = "", pay = "";
		var tr=0, check=0, checkAcc=0, checkTT=0, checkCat=0, checkPay=0;
		...
		$.get(location.protocol + "//" + location.host + "/FinanceApp/Files/Offline/transactions_"+cid+".xml",function(data){
			var htmlOutputStr = '';
	    	htmlOutputStr += '<table><tr>';
			htmlOutputStr += '<th>Del</th>';
			...
			$(data).find('transaction').each(function(){
				if(($(this).find("enable").text())*1 > 0){
					...
					if(check===1){
						...
						htmlOutputStr += '<tr>';
			    		htmlOutputStr += '<td class="column1"><a href="#" class="disableRow">'+$(this).attr("tID")+'</a></td>';
    		...
    		htmlOutputStr += '</table>';
   			$("#transactionsXML_Offline").html(htmlOutputStr);
			$('#resultSummary').html('<p>transactions returned: '+tr+'</p><h3>Filters used:</h3>');
